...
void
MeshWifiInterfaceMac::SetDcfTxStartCallback (Dcf::TxStart callback)
{
  for (Queues::const_iterator i = m_queues.begin (); i != m_queues.end (); i++)
    {
      i->second->SetTxStartCallback (callback);
    }
  m_beaconDca->SetTxStartCallback (callback);
}

Ptr<MacLow>
MeshWifiInterfaceMac::GetMacLow (void)
{
  return m_low;
}
