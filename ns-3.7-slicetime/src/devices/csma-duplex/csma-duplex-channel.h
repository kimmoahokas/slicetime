...
// this file is an adapted version of devices/csma/csma-channel.h

#ifndef CSMA_DUPLEX_CHANNEL_H
#define CSMA_DUPLEX_CHANNEL_H
 ...
 */
class CsmaDuplexDeviceRec {
public:
  Ptr< CsmaDuplexNetDevice > devicePtr; /// Pointer to the net device
  bool                       active;    /// Is net device enabled to TX/RX

  CsmaDuplexDeviceRec();
  CsmaDuplexDeviceRec(Ptr< CsmaDuplexNetDevice > device);
...
/**
 * \brief Csma Duplex Channel.
 *
 * This class represents a simple duplex Csma channel that can be used
 * when exactly two nodes are connected to one wire. Its difference to 
 * CsmaChannel is that it uses two different busy
 * flags to indicate if the channel is currently in use. It does not
 * take into account the distances between stations or the speed of
 * light to determine collisions.
 */
class CsmaDuplexChannel : public Channel 
{
public:
  static TypeId GetTypeId (void);

  /**
   * \brief Create a CsmaDuplexChannel
   */
  CsmaDuplexChannel ();
   ...
   */
  int32_t Attach (Ptr<CsmaDuplexNetDevice> device);
   ...
   * can't be found.
   */
  bool Detach (Ptr<CsmaDuplexNetDevice> device);
   ...
   * channel or can't be found.
   */
  bool Reattach(Ptr<CsmaDuplexNetDevice> device);
   ...
   * number is needed
   */
  int32_t GetDeviceNum (Ptr<CsmaDuplexNetDevice> device);
  ...
  /**
   * \return Get a CsmaDuplexNetDevice pointer to a connected network device.
   *
   ...
   * with deviceId i.
   */
  Ptr<CsmaDuplexNetDevice> GetCsmaDevice (uint32_t i) const;
...
private:
   ...
   */
  std::vector<CsmaDuplexDeviceRec> m_deviceList;
   ...
   * free.)
   */
  Ptr<Packet> m_currentPkt0;
  Ptr<Packet> m_currentPkt1;
   ...
   */
  uint32_t                            m_currentSrc0;
  uint32_t                            m_currentSrc1;
