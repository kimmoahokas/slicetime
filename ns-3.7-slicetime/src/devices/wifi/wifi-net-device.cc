...
WifiNetDevice::SetMtu (const uint16_t mtu)
{
  ...
  if (mtu > maxMsduSize.Get () - 8 || mtu == 0) // maxMsdu includes the LLC header which is added later-on
    {
      return false;
    }
  m_mtu = mtu;
  ...
  if (m_mtu == 0)
    {
      UintegerValue maxMsduSize;
      m_mac->GetAttribute ("MaxMsduSize", maxMsduSize);
      m_mtu = maxMsduSize.Get () - 8; // maxMsdu includes the LLC header which is added later-on
    }
  return m_mtu;
}
bool 
...
void
WifiNetDevice::StartActiveScanning (std::vector<uint16_t> const &channels,
                                    WifiMac::ScanningCallback callback)
{
  m_mac->StartActiveScanning (channels, callback, this);
}
