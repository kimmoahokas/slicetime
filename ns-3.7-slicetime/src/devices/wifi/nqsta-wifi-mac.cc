 ...
 * Author of Scanning: Gustavo Carneiro <gjc@inescporto.pt>
 */
// scanning functionality from http://www.nsnam.org/contributed/ns-3-wifi-scanning.tar.bz2
 ...
 * The state machine for this NQSTA is:
 ...
 Additions for scanning:

 Associated -----\
 Beacon Missed ---+---> Scanning ---> (original state)
 Refused---------/
...
void
NqstaWifiMac::SetDcfTxStartCallback (Dcf::TxStart callback)
{
  m_dca->SetTxStartCallback(callback);
}

Ptr<MacLow>
NqstaWifiMac::GetMacLow (void)
{
  return m_low;
}
...
NqstaWifiMac::TryToEnsureAssociated (void)
{
  ...
  case BEACON_MISSED:
     ...
     */
    SetBssid(Mac48Address());
    if (!m_linkDown.IsNull ())
      {
        m_linkDown ();
      }
  ...
  case REFUSED:
    ...
    break;
  case SCANNING:
    NS_FATAL_ERROR ("Cannot associate while scanning");
    break;
  }
}
  ...
  else if (hdr->IsProbeResp ()) 
    {
      if (m_state == WAIT_PROBE_RESP) 
      ...
      else if (m_state == SCANNING)
        {
          ...
          WifiSignalNoiseTag tag;
          packet->PeekPacketTag(tag);
          
          ScanningEntry entry;
          entry.channel = m_phy->GetChannelNumber ();
          entry.ssid = probeResp.GetSsid ();
          entry.bssid = hdr->GetAddr3 ();
          entry.signal = tag.GetSignal ();
          entry.noise = tag.GetNoise ();
          
          m_scanning.results.push_back (entry);
        }
    }
...
void NqstaWifiMac::ActiveScanningChannelTimeout ()
{
  Ptr<WifiNetDevice> netdevice = m_scanning.netdevice->GetObject<WifiNetDevice> ();
  if (*m_scanning.nextChannelIter == *m_scanning.channelsToScan.end ())
    {
      NS_LOG_LOGIC ("ActiveScanning ends; switching back to the original channel, then calling the callback.");
      // that's it, we're done scanning, send the results to the client
      m_phy->SetChannelNumber (m_scanning.channelBeforeScanning);      
      m_state = m_scanning.stateBeforeScanning;
      m_scanning.callback (m_scanning.results);
    }
  else
    {
      NS_LOG_LOGIC ("ActiveScanning continues to channel " << *m_scanning.nextChannelIter);
      m_phy->SetChannelNumber(*m_scanning.nextChannelIter);
      m_scanning.nextChannelIter++;
      m_scanning.timer.Schedule ();
...
void NqstaWifiMac::StartActiveScanning (std::vector<uint16_t>  const &channels,
                                        ...
                                        Ptr<NetDevice> netdevice)
{
  switch (m_state)
    ...
    case SCANNING:
      break;
    default:
      NS_FATAL_ERROR ("Cannot do scanning in state " << m_state);
    }
  m_scanning.netdevice = netdevice;
  m_scanning.callback = callback;
  m_scanning.channelBeforeScanning = m_phy->GetChannelNumber ();
  m_scanning.stateBeforeScanning = m_state;
  m_scanning.channelsToScan = channels;
  NS_ASSERT (channels.size () > 0);
  m_scanning.nextChannelIter = m_scanning.channelsToScan.begin ();
  m_scanning.results = std::vector<ScanningEntry> ();
  m_scanning.timer.SetFunction (&NqstaWifiMac::ActiveScanningChannelTimeout, this);
  m_scanning.timer.SetDelay (m_probeRequestTimeout);
  m_state = SCANNING;
  NS_LOG_DEBUG (Simulator::Now () << ": starting active scan");
  ActiveScanningChannelTimeout ();
}
