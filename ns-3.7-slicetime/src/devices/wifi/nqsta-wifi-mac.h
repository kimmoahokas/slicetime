...
public:
  ...
  virtual void SetDcfTxStartCallback (Dcf::TxStart callback);
  virtual Ptr<MacLow> GetMacLow (void);
   ...
   * \param callback callable to call when all channels have been scanned
   *
   * Start an active scanning process over the specified channels.
   * When the scanning is complete, the specified callback will be
   * called with the scan results.
   */
  virtual void StartActiveScanning (std::vector<uint16_t>  const &channels,
...
private:
  ...
  void ActiveScanningChannelTimeout ();
  ...
  struct
  {
    Ptr<NetDevice> netdevice;
    // input
    ScanningCallback callback; // scanning client callback
    std::vector<uint16_t> channelsToScan;
    // output
    std::vector<ScanningEntry> results;
    // other state
    uint16_t channelBeforeScanning;
    enum MacState stateBeforeScanning;
    std::vector<uint16_t>::iterator nextChannelIter;
    Timer timer; // timer that controls the amount of time spent on each channel
  } m_scanning;
