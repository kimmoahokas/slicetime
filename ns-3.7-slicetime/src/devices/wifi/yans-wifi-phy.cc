  ...
  if (m_random.GetValue () > snrPer.per) 
    {
      ...
      // try to remove existing WifiSignalNoiseTag
      // (can occur if access point forwards a frame)
      WifiSignalNoiseTag dummyTag;
      packet->RemovePacketTag(dummyTag);
      // add tag containing signal and noise
      WifiSignalNoiseTag tag (signalDbm, noiseDbm);
