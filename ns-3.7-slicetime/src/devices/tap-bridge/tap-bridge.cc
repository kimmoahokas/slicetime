  ...
  for (;;) 
    {
      ...
      //
      // TapBridge requires a special SimulatorImpl.
      ...
      EventImpl *event = MakeEvent (&TapBridge::ForwardToBridgedDevice, this, buf, len);
      uint32_t node_id = GetNode ()->GetId (); 

      if (m_rtImpl != NULL)
        {
          NS_LOG_INFO ("TapBridge::ReadThread(): Scheduling handler in RealtimeSimulatorImpl");
          m_rtImpl->ScheduleRealtimeNowWithContext (node_id, event);
        }
      else if (m_syncImpl != NULL)
        {
          NS_LOG_INFO ("TapBridge::ReadThread(): Scheduling handler in SyncSimulatorImpl");
          m_syncImpl->ScheduleInCurrentSliceWithContext (node_id, event);
        }
      else
        {
          NS_FATAL_ERROR("TapBridge::ReadThread(): TapBridge has to be used with RealtimeSimulatorImplementation or SyncSimulatorImplementation!");
        }
