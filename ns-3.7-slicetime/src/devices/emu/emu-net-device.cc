...
EmuNetDevice::StartDevice (void)
{
  ...
  //
  // In case of synchronized emulation, a pointer of type SyncSimulatorImpl is needed.
  // Right here we simply get pointers of both types and check later on which of them is NULL.
  // This is necessary since these SimulatorImpls provide different methods. 
  //
  ...
  for (;;) 
    {
      ...
      //
      // EmuNetDevice requires a special SimulatorImpl.
      ...
      EventImpl *event = MakeEvent (&EmuNetDevice::ForwardUp, this, buf, len);
      uint32_t node_id = GetNode ()->GetId (); 

      if (m_rtImpl != NULL)
        {
          NS_LOG_INFO ("EmuNetDevice::ReadThread(): Scheduling handler in RealtimeSimulatorImpl");
          m_rtImpl->ScheduleRealtimeNowWithContext (node_id, event);
        }
      else if (m_syncImpl != NULL)
        {
          NS_LOG_INFO ("EmuNetDevice::ReadThread(): Scheduling handler in SyncSimulatorImpl");
          m_syncImpl->ScheduleInCurrentSliceWithContext (node_id, event);
        }
      else
        {
          NS_FATAL_ERROR("EmuNetDevice::ReadThread(): EmuNetDevice has to be used with RealtimeSimulatorImplementation or SyncSimulatorImplementation!");
        }
